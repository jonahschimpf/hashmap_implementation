...
/**
 * Implementation exercise for a {@link Map} using hashing. The final result
 * will be an implementation with characteristics similar to the OpenJDK
 * implementation.
 *
 */
public class HashMap<K, V> extends BaseAbstractMap<K, V> {
     ...
     */
    public static int hash(int h, int length) {
    ...
    public V put(K key, V value) {
        ...
        if (key == null) {
            if (table[0] == null) {
                ...
                while (e.key != null) {
                    if (e.next == null) {
                        Entry<K, V> newE = new Entry<K, V>(key, value, table[0]);
                        table[0] = newE;
                        size++;
                        return null; // no previous mapping for key so should return null
                    } else {
                        e = e.next;
                    }
                }
            ...
            if (table[hashKey] == null) {
                ...
                while (e.key == null || !(e.key.equals(key))) {
                    if (e.next == null) {
                        Entry<K, V> newE = new Entry<K, V>(key, value, table[hashKey]);
                        table[hashKey] = newE;
                        size++;
                        return null; // no previous mapping for key so should return null
                    } else {
    ...
    public void clear() {
        ...
        // you can definitely use a table of smaller size, as long as it's a power of
        // two.
    ...
    protected Iterator<Map.Entry<K, V>> entryIterator() {
        return (Iterator<java.util.Map.Entry<K, V>>) new MapIterator();
    }

    @SuppressWarnings("hiding")
    private class MapIterator<E> implements Iterator<E> {
        private LinkedList<Entry<K, V>> s;
        boolean[] added;

        public MapIterator() {
            s = new LinkedList<Entry<K, V>>();
            added = new boolean[table.length];
            for (int i = 0; i < table.length; i++) {
                if (table[i] != null) {
                    s.add(table[i]);
                    added[i] = true;
